use anyhow::{Result};
use std::{fs::{self}, path::Path, time::{SystemTime, UNIX_EPOCH}, env};
use std::collections::HashMap;
use serde::{Deserialize, Serialize};
use serde_jsonlines::{json_lines, append_json_lines};
...
fn config() -> Config {
    Config { wal_path: default_wal_path() }
}
...
pub fn add<P: AsRef<Path>>(key: String, value: String, wal_path: Option<P>) -> Result<(String, String)> {
...
pub fn read_from_memory<P: AsRef<Path>>(key: String, wal_path: Option<P>) -> Result<(String, String), StoreError> {
...
mod tests {
  ...
  fn all_returns_kvs_in_wal_log() -> () {
    ...
    let contents = format!("{},{:?},{},{},{},{}", "CRC".to_string(), duration.as_millis(), key_size.to_string(), value_size.to_string(), key, value_string);
    let contents2 = format!("{},{:?},{},{},{},{}", "CRC".to_string(), duration.as_millis(), key_size.to_string(), value_size.to_string(), key, value_string);
    writeln!(file, "{}\n{}", contents, contents2).unwrap();

    assert_matches_regex!(all(Some(file_path)).unwrap(), r"CRC,\d+,1,12,a,Hello World!")
  }
  ...
  fn read_from_memory_returns_the_keyed_value_when_it_exists() -> (){
    ...
    assert_eq!(result.unwrap(), (String::from("B"), String::from("Man")), "incorrect value")
  }
  ...
  fn read_from_memory_returns_an_err_when_it_doesnt_exist() -> (){
    ...
    // assert_eq!(result, String::from("Not found"), "incorrect value")
